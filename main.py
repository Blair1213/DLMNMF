...
from config import ModelConfig, PROCESSED_DATA_DIR,  ENTITY_VOCAB_TEMPLATE, \
    ...
    DRUG_VOCAB_TEMPLATE,FEATURE_TEMPLATE
...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
    ...
    config.feature = np.load(format_filename(PROCESSED_DATA_DIR,FEATURE_TEMPLATE ,
                                                  dataset=dataset))

    config.exp_name = f'DLCMNMF_{dataset}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
