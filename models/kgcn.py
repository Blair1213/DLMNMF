...
from sklearn.metrics import roc_auc_score, accuracy_score, f1_score, precision_recall_curve,recall_score, precision_score
...
class KGCN(BaseModel):
    ...
    def build(self):
        input_drug = Input(
            #drug_one smile
            shape=(1, ), name='input_drug', dtype='int64')
        input_virus = Input(
            #drug_two_smile
            shape=(1, ), name='input_virus', dtype='int64')
        ...
        ##获取embedding
        drug_embedding = Lambda(lambda x:self.get_embedding(x),name="get_drug_embedding")(input_drug)
        virus_embedding = Lambda(lambda x:self.get_embedding(x),name="get_virus_embedding")(input_virus)

        receptive_list_drug_one = Lambda(lambda x: self.get_receptive_field(x),
                                         name='receptive_filed_drug_one')(input_drug)
        ...
        neigh_ent_embed_list_drug_one = [Lambda(lambda x: self.get_embedding(x))(
        ...
        for depth in range(self.config.n_depth):
            ...
            for hop in range(self.config.n_depth-depth):
                neighbor_embed = neighbor_embedding([drug_embedding, neigh_rel_embed_list_drug_one[hop],
        ...
        for depth in range(self.config.n_depth):
            ...
            for hop in range(self.config.n_depth-depth):
                neighbor_embed = neighbor_embedding([virus_embedding, neigh_rel_embed_list[hop],
        ...
        drug_squeeze_embed = Lambda(lambda x: K.squeeze(
            x, axis=1))(neigh_ent_embed_list_drug_one[0])
        virus_squeeze_embed = Lambda(lambda x: K.squeeze(
            x, axis=1))(neigh_ent_embed_list[0])
        drug_virus_score = Lambda(
            lambda x: K.sigmoid(K.sum(x[0] * x[1], axis=-1, keepdims=True))
        )([drug_squeeze_embed, virus_squeeze_embed])

        model = Model([input_drug, input_virus], drug_virus_score)
    ...
    def get_embedding(self,entity_id):
        ...
        node_feature = K.variable(
            self.config.feature, name='node_feature')

        entity_feature = K.gather(node_feature, K.cast(
            entity_id, dtype='int64'))  # get drug's hash
